...
import static rule.utils.SkChar.isBadMacroChar;
import static rule.utils.SkChar.isQuote;
...
public class SkExpressionFactory {
    ...
    public static SkExpression parseExpression(final String inInputExpression) {
        ...
        String macro = "";
        final List<String> macroList = Lists.newArrayList();
    ...
    private static String convertExpression(final String inInputExpression, final StringBuilder inSb,
        ...
        for (int i = 0; i < inInputExpression.length(); i++) {
            final char c = inInputExpression.charAt(i);
            if (!inHaveMacro) {
                if (SkChar.isAlpha(c)) {
                    inHaveMacro = true;
                }
     ...
     */
    private static boolean isNonMacro(final String inInputExpression, int inIndex) {
