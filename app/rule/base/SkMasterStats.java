...
public class SkMasterStats {
    ...
    public SkAction getAction(final String inActionName) {
        if (null != inActionName) {
            final String actionNameUpper = inActionName.toUpperCase();
            final SkAction action = this.actionNameMap.get(actionNameUpper);
            if (null == action) {
    ...
    public SkRule getRule(String inRuleName) {
        if (null != inRuleName) {
            final SkRule rule = this.ruleNameMap.get(inRuleName);
            if (null == rule) {
    ...
    private void buildMaps() {
        ...
        for (SkAction action : actionList) {
            final String actionName = getActionNameForMaps(action, index);
    ...
    SkIf findIfFromReference(String inRuleName) {
        if (null != inRuleName) {
            final SkRule rule = this.ruleNameMap.get(inRuleName.toUpperCase());
            if (null != rule) {
    ...
    private void mapsFromRule(final AtomicInteger inIndex, final SkRule rule) {
        final String name = getRuleNameForMaps(rule, inIndex);
    ...
    private void mapsFromRuleIfs(final SkRule rule, final String inName) {
        ...
        // Rule/Macro Map
        // Macro/SkIf Map
        final SkCondition condition = rule.getCondition();
        SkIf skIf = null;
        ...
        if (null != skIf) {
            final SkExpression exp = skIf.getSkExpression();

            if (null != exp) {
                final List<String> macroList = exp.getMacroList();
     ...
     * @return
     */
    private String getRuleNameForMaps(final SkRule inRule, final AtomicInteger inIndex) {
        String name = inRule.getName();
        if (null == name || 0 == name.length()) {
            final int i = inIndex.incrementAndGet();
     ...
     * @return
     */
    private String getActionNameForMaps(final SkAction inAction, final AtomicInteger inIndex) {
